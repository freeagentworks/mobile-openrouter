...
* **モデル切り替え:**
    * **デフォルトモデル:**
        * テキストのみの会話では、`openai/gpt-oss-120b:free` を使用する。
        * 画像が添付された会話では、`meta-llama/llama-4-maverick:free` を使用する。
    * **ユーザーによるモデル選択:**
        * ユーザーがチャット画面のUIから、利用するLLMを自由に選択・切り替えできるドロップダウンメニュー等を設置する。
        * 選択可能なモデルリストは、OpenRouterが提供するモデルの中から選定する。

### 4.2. 設定機能

* **設定インターフェース:**
    * 設定画面で、ユーザーが自身のOpenRouter APIキーやLLMのパラメータ（Temperatureなど）を入力・設定できる。
* **データ保存:**
    * APIキーや設定値は、ブラウザの`localStorage`を使用してユーザーのローカル端末に保存する。サーバーへの送信は一切行わない。
* **初期状態:**
    * APIキーが未設定の場合、チャット機能利用時に設定を促すメッセージを表示する。
* **エラーハンドリング:**
    * APIキーが未設定、または不正である場合、API通信時に「APIキーが設定されていないか、間違っています。」といった内容をポップアップダイアログで表示する。
